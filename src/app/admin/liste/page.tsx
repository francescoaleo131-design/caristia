      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1 max-w-md">
            Gestisci i desideri dei piccoli festeggiati e monitora i regali ricevuti.
          </p>
        </div>
        ...
        {liste && liste.length > 0 ? (
          liste.map((lista) => (
            <div key={lista.id} className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all group">
              ...
              {/* Barra di progressione (Esempio basato su una percentuale ipotetica) */}
              <div className="space-y-1.5 mb-6">
                <div className="flex justify-between text-xs font-medium">
                  <span className="text-slate-400">Progresso regali</span>
                  <span className="text-indigo-600">{lista.progress || 0}%</span>
                </div>
                <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
                  <div 
                    className="h-full bg-indigo-500 rounded-full transition-all duration-500" 
                    style={{ width: `${lista.progress || 0}%` }}
                  ></div>
                </div>
              </div>
