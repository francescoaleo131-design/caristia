...
export default function FormContatti() {
  const [formData, setFormData] = useState({
    nome: "", telefono: "", bambini: "", tipoFesta: "Compleanno", data: "", messaggio: ""
  })

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    const numeroOwner = "393714563422" 
    ...
    <section id="contatti" className="py-5 bg-white scroll-mt-20">
      <div className="container mx-auto px-4 max-w-3xl">
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-8 rounded-3xl shadow-xl">
          ...
          <select name="tipoFesta" className="p-3 bg-gray-50 border rounded-xl outline-none" onChange={handleChange}>
          ...
          <textarea name="messaggio" rows={3} placeholder="Note speciali..." className="md:col-span-2 p-3 bg-gray-50 border rounded-xl outline-none" onChange={handleChange} />
          <button type="submit" className="md:col-span-2 bg-[#1e73be] hover:bg-[#8cc665] text-white font-black py-4 rounded-2xl uppercase tracking-widest transition-all flex items-center justify-center gap-2">
